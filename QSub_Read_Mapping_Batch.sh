...
#       2015-06-02
#           Added support for for-looped read mapping and running one command for all samples
#       2015-06-04
#           Created generallized version of script, runs as a for-loop
...
do
    ...
    SAMPLE=`basename "$f" "$EXT"`
    $PROGRAM mem -t 8 -k 10 -r 1.0 -M -T 85 -O 8 -E 1\
        ${REF_GEN} \
        ${f} \
        ${r} > \
        ${SCRATCH}/${PROJECT}/${SAMPLE}_${PROJECT}_${YMD}.sam
done
